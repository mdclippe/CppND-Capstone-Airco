...
MainWindow::MainWindow( Monitor&& moni, Airco& airco, TemperatureNode& node, QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::MainWindow)
    , _monitor(moni), _airco(airco), _temperatureNode(node)
{
    ui->setupUi(this);
    QObject::connect(&_monitor, &Monitor::targetTemperatureChanged, ui->lcdTargetTemp,QOverload<double>::of(&QLCDNumber::display));
    QObject::connect(&_monitor, &Monitor::currentTemperatureChanged, ui->lcdCurrentTemp,QOverload<double>::of(&QLCDNumber::display));
    QObject::connect(&_monitor, &Monitor::aircoStatusChanged, this, &MainWindow::updateStatus);
